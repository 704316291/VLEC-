...
import {GetFAQListByType} from "../../api/api";
                                        ...
                                        <StickyContainer>
                                            <Tabs defaultActiveKey="0" renderTabBar={renderTabBar}
                                                ...
                                                {this.state.TypeData.map((item, i) => {
                                                    return <TabPane tab={item.Text} key={i} style={{height: 500}}
                                                                    value={item.Value}>
                                                        <Collapse bordered={false} defaultActiveKey={['1']}>
                                                            {this.state.ListByType.map((item) => {
                                                                return <Panel header={`Q : ${this.props.local === "en" ? item.TitleEN : item.TitleCN}`}
                                                                    key={item.ID} style={{color: "red !important"}}>
                                                                     <span style={{color:"#007aff",fontWeight:"bold"}}> A : </span>{this.props.local === "en" ? item.TitleEN : item.TitleCN}
                                                            </Panel>
                                                            })}
                                                        </Collapse>
                                                    </TabPane>
